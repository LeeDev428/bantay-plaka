...
start "BantayPlaka - ENTRY CAM" cmd /k "venv\Scripts\activate.bat ^&^& python anpr_engine/anpr_engine.py --rtsp ""%ENTRY_RTSP%"" --camera-role ENTRY_CAM"

echo [3/3] Starting EXIT camera ANPR (TIME_OUT mapping)...
start "BantayPlaka - EXIT CAM" cmd /k "venv\Scripts\activate.bat ^&^& python anpr_engine/anpr_engine.py --rtsp ""%EXIT_RTSP%"" --camera-role EXIT_CAM"
